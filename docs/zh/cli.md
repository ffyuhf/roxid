...
- `--help` 文本与参数解析报错均跟随 locale 双语显示（`LC_ALL` → `LC_MESSAGES` → `LANG`，`zh*` 前缀 → 中文，其余 → 英文）：报错含错误行、用法行与提示行；help 段落标题（用法：/选项：/参数：/命令：）与 `-h` 说明同步本地化；参数说明统一「独立成行」布局（迭代47）。
...
| `-i, --interactive` | flag | 与 `-f` 二选一 | — | 全屏 TUI 向导：① 列表选基础模型（↑/↓ 移动、输入即过滤）→ ② SYSTEM 多行编辑（Enter 完成、Alt+Enter 换行、粘贴自动保留换行、内容超出屏高自动滚动跟随光标）→ ③ RUNTIME 参数（示例常驻底部）→ ④ Modelfile 预览确认（↑/↓ 滚动、PgUp/PgDn 十行）。终端不支持全屏时自动降级为逐行问答向导 |
...
|---|---|
| `0` | 成功 |
| `1` | 失败：服务端错误响应 / 文件读写失败 / 交互向导取消等 |
| `2` | 参数解析错误（GNU 惯例：缺必选参数 / 未知参数 / 未识别子命令等，报错文案跟随 locale；迭代47 修正与代码实际行为一致） |
